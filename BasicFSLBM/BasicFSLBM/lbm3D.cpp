...
///
static inline void DFD3Q19_DeriveQuantities(LBM::dfD3Q19_t *df)
{
	int i;
...
void DFD3Q19_Init(const int type, const real f[19], LBM::dfD3Q19_t *df)
{
	...
	memset(df, 0, sizeof(LBM::dfD3Q19_t));
...
void LBM3DField_Allocate(const real omega, LBM::lbm_field3D_t *field)
{
	...
	field->df = (LBM::dfD3Q19_t *)calloc(SIZE_3D_X*SIZE_3D_Y*SIZE_3D_Z, sizeof(LBM::dfD3Q19_t));
}
...
///
static inline void CalculateLBMCollisionStep3D(const LBM::lbm_field3D_t *restrict_ f0, const vec3_t gravity, LBM::lbm_field3D_t *restrict_ f_coll)
{
	int x, y, z;
	int i;
	...
	for (z = 0; z < SIZE_3D_Z; z++)
	{
		for (y = 0; y < SIZE_3D_Y; y++)
		{
			...
			{
				// current cells
				const LBM::dfD3Q19_t *df = LBM3DField_Get(f0, x, y, z);
				LBM::dfD3Q19_t *df_coll = LBM3DField_Get(f_coll, x, y, z);
...
///
static inline real DFD3Q19_CalcEpsilon(const LBM::dfD3Q19_t *df)
{
...
///
static inline vec3_t LBM3DField_CalcNormal(const LBM::lbm_field3D_t *field, const int x, const int y, const int z)
{
	// Eq. (4.6)
	vec3_t normal;
...
///
static inline void CalculateLBMStreamStep3D(const LBM::lbm_field3D_t *restrict_ f0, LBM::lbm_field3D_t *restrict_ f_strm)
{
	int x, y, z;
	int i;
	...
	for (z = 0; z < SIZE_3D_Z; z++)
	{
		for (y = 0; y < SIZE_3D_Y; y++)
		{
			...
			{
				const LBM::dfD3Q19_t *df = LBM3DField_Get(f0, x, y, z);
				LBM::dfD3Q19_t *df_strm = LBM3DField_Get(f_strm, x, y, z);
				...
				if (df->type & CT_FLUID)
				{
					...
					{
						// neighbor cell
						const LBM::dfD3Q19_t *df_neigh = LBM3DField_GetMod(f0, x - vel3Di[i][0], y - vel3Di[i][1], z - vel3Di[i][2]);
...
///
static inline void LBM3DField_AverageSurrounding(const LBM::lbm_field3D_t *field, const int x, const int y, const int z, LBM::dfD3Q19_t *df)
{
	int i;
	int n = 0;	// counter
	...
	for (i = 1; i < 19; i++)		// omit zero vector
	{
		LBM::dfD3Q19_t *df_neigh = LBM3DField_GetMod(field, x - vel3Di[i][0], y - vel3Di[i][1], z - vel3Di[i][2]);
...
///
static inline void UpdateTypesLBMStep3D(const LBM::lbm_field3D_t *restrict_ f0, LBM::lbm_field3D_t *restrict_ f_distr, LBM::lbm_field3D_t *restrict_ f_next)
{
...
#define LONELY_THRESH		((real)0.1)
	...
	for (z = 0; z < SIZE_3D_Z; z++)
	{
		for (y = 0; y < SIZE_3D_Y; y++)
		{
			for (x = 0; x < SIZE_3D_X; x++)
			{
				// current cell
				const LBM::dfD3Q19_t *df = LBM3DField_Get(f0, x, y, z);
				LBM::dfD3Q19_t *df_next = LBM3DField_Get(f_next, x, y, z);
	...
	for (z = 0; z < SIZE_3D_Z; z++)
	{
		for (y = 0; y < SIZE_3D_Y; y++)
		{
			...
			{
				// current cell
				LBM::dfD3Q19_t *df = LBM3DField_Get(f_next, x, y, z);
				...
				if (df->type & CT_IF_TO_FLUID)
				{
					...
					for (i = 1; i < 19; i++)		// omit zero vector
					{
						...
						LBM::dfD3Q19_t *df_neigh = LBM3DField_GetMod(f_next, x_neigh, y_neigh, z_neigh);
					...
					{
						// neighbor cell
						LBM::dfD3Q19_t *df_neigh = LBM3DField_GetMod(f_next, x - vel3Di[i][0], y - vel3Di[i][1], z - vel3Di[i][2]);
	...
	for (z = 0; z < SIZE_3D_Z; z++)
	{
		for (y = 0; y < SIZE_3D_Y; y++)
		{
			for (x = 0; x < SIZE_3D_X; x++)
			{
				...
				LBM::dfD3Q19_t *df_distr = LBM3DField_Get(f_distr, x, y, z);
			...
			for (x = 0; x < SIZE_3D_X; x++)
			{
				...
				if (df->type & CT_INTERFACE)
				{
					for (i = 1; i < 19; i++)		// omit zero vector
					{
						...
						LBM::dfD3Q19_t *df_distr = LBM3DField_GetMod(f_distr, x - vel3Di[i][0], y - vel3Di[i][1], z - vel3Di[i][2]);
...
void CalculateLBMStep3D(const LBM::lbm_field3D_t *restrict_ f0, const vec3_t gravity, LBM::lbm_field3D_t *restrict_ f1)
{
	...
	LBM::lbm_field3D_t f_coll, f_strm, f_distr;
...
void LatticeBoltzmann3DEvolution(const LBM::lbm_field3D_t *restrict_ startfield, const int numsteps, const vec3_t gravity, LBM::lbm_field3D_t *restrict_ fieldevolv)
{
	...
	memcpy(fieldevolv->df, startfield->df, SIZE_3D_X*SIZE_3D_Y*SIZE_3D_Z*sizeof(LBM::dfD3Q19_t));
...
{
	for ( lbm.frame = 1; lbm.frame < numsteps; lbm.frame++)
	{
		LBM3DField_Allocate(startfield->omega, &fieldevolv[lbm.frame]);

		CalculateLBMStep3D(&fieldevolv[lbm.frame - 1], gravity, &fieldevolv[lbm.frame]);
		lbm.DrawFluid();
		printf("%d, ", lbm.frame);
	}
