...
void LBM::DrawFluid(void)
{
	...
	if (rendermode == RENDER_HEAT3D)renderheat_3D();
	if (rendermode == RENDER_HEATPLANE)renderheat_plane();
	if (rendermode == RENDER_VELOCITY)render_vel();
	if (rendermode == RENDER_DENSITY)render_dens();
	glutPostRedisplay();
	glutSwapBuffers();
}
...
void LBM::renderheat_plane()
{
	...
	for (k = 0; k < NZ; k++)
	{
...

		{
			...
			// 			glEnd();
			glBegin(GL_QUADS);
			glVertex3f((float)i - 0.4, (float)0.75*NY, (float)k - 0.4);
			glVertex3f((float)i - 0.4, (float)0.75*NY, (float)k + 0.4);
			glVertex3f((float)i + 0.4, (float)0.75*NY, (float)k + 0.4);
			glVertex3f((float)i + 0.4, (float)0.75*NY, (float)k - 0.4);
			glEnd();
...
void LBM::render_dens()
{
	...
	for (k = 1; k < NZ; k++)
	{
...

		{
			...
			// 			glEnd();
			glBegin(GL_QUADS);
			glVertex3f((float)i - 0.45, (float)0.75*NY, (float)k - 0.45);
			glVertex3f((float)i - 0.45, (float)0.75*NY, (float)k + 0.45);
			glVertex3f((float)i + 0.45, (float)0.75*NY, (float)k + 0.45);
			glVertex3f((float)i + 0.45, (float)0.75*NY, (float)k - 0.45);
			glEnd();
...
void LBM::outputuxyz()
{
	...
	if (frame % 10 == 0)
	printf("outputfile:%i  center T:%f V:%f: total E:%f \n", frame, h_T[getid(NX/2, NY/2, NZ/2)], h_vz[getid(NX / 8,NY / 2,NZ / 2)], total_E);
